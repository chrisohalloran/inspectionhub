...
and blocked-manifest exit behavior. They also reject stale evidence envelopes,
physical app builds and reviews whose commits do not exactly match the run,
reject deterministic-only or exposed-holdout AI evidence, recompute release
outcomes from 120 exact case/architecture/trial records, preserve schema-valid
historical-input migration with literal blockers, and prevent artifact-readback
bypass from creating a completion event.
...
- No physical Build Week iPhone full journey and offline termination/recovery
  artifact has been supplied. A current physical build reached Xcode's signing
  stage for the connected iPhone but failed because this Xcode installation has
  no signed-in developer account and no provisioning profile for
  `co.inspectionhub.field`; simulator proof is not relabelled as device proof.
...
- Parallel adversarial implementation, security and document review completed
  with zero unresolved P0/P1. The final model-assisted review boundary and
  resolved findings are recorded in `adversarial-review-2026-07-17.md`; it is
  not represented as an external human review.
- The repository and review observations recorded on 16 July are bound to
  their historical source commit. They must be refreshed after the final code
  change; the collector now rejects rather than relabels them onto a later run.
...
- An explicit-clean-state six-flow Maestro run passed sequentially on the
  dedicated iOS 26.3 simulator. It covers termination recovery, local photo/voice/offline
  capture, exact review and module approvals, manual-note/source-packet lineage,
  recipient-safe projection and cached-job capture after session expiry. It is
  local UI proof only.
...
The latest default 17 July run remained truthfully blocked at 0% with zero
validation errors. A default evidence input intentionally does not infer local
test output, checked repository prose or review prose as observed milestone
evidence.
